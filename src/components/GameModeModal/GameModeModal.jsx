...
import { 
  ...
  ArrowRight, ArrowLeft, GraduationCap, Trophy, ChevronRight, HelpCircle, BookOpen, Trash2, RotateCcw, Clock 
} from 'lucide-react';
...
export const GameModeModal = ({
  ...
  const content = (
    <div
      className={embedded ? "play-hub-card" : "modal-card"}
      style={{
        maxWidth: '860px',
        width: '100%',
        ...
        boxShadow: embedded ? '0 8px 30px rgba(0, 0, 0, 0.4)' : '0 24px 60px rgba(0, 0, 0, 0.7)',
        maxHeight: embedded ? 'none' : '92vh',
        overflowY: embedded ? 'visible' : 'auto'
      }}
      onClick={(e) => e.stopPropagation()}
    >
      {/* BANNER DE PARTIDA EN CURSO DISPONIBLE */}
      {ongoingGame && !selectedOpponent && (
        <div style={{
          background: 'linear-gradient(135deg, rgba(234, 179, 8, 0.22) 0%, rgba(59, 130, 246, 0.25) 100%)',
          border: '2px solid var(--color-gold)',
          borderRadius: '14px',
          padding: '16px 20px',
          marginBottom: '20px',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '46px', height: '46px', borderRadius: '12px', background: 'var(--color-gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.6rem', flexShrink: 0 }}>
              ♟️
            </div>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px' }}>
                <span style={{ background: 'var(--color-gold)', color: '#000', padding: '2px 8px', borderRadius: 'var(--radius-full)', fontSize: '0.72rem', fontWeight: '900' }}>
                  PARTIDA EN CURSO DISPONIBLE
                </span>
                <span style={{ fontSize: '0.76rem', color: 'var(--text-parchment-muted)' }}>
                  Turno {Math.floor((ongoingGame.moveHistory?.length || 0) / 2) + 1} • {ongoingGame.gameMode === 'pass_and_play' ? '2 Jugadores Local' : (ongoingGame.botName || 'vs Robot')}
                </span>
              </div>
              <div style={{ fontSize: '1.05rem', fontWeight: '900', color: 'var(--text-parchment-main)' }}>
                {ongoingGame.gameMode === 'pass_and_play' 
                  ? 'Partida Local (Pasa y Juega en este dispositivo)' 
                  : `Partida contra ${ongoingGame.botName || 'Robot'}`}
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
            {onResumeOngoingGame && (
              <button
                type="button"
                className="btn-gold"
                onClick={onResumeOngoingGame}
                style={{ padding: '10px 18px', fontSize: '0.90rem', fontWeight: '900', gap: '6px' }}
              >
                <Play size={16} />
                <span>Continuar Partida ▶</span>
              </button>
            )}
            {onDiscardOngoingGame && (
              <button
                type="button"
                className="btn-secondary"
                onClick={onDiscardOngoingGame}
                style={{ padding: '10px 14px', fontSize: '0.82rem', color: '#ef4444', borderColor: 'rgba(239, 68, 68, 0.35)' }}
                title="Descartar partida actual para comenzar una nueva"
              >
                <Trash2 size={15} />
                <span>Descartar</span>
              </button>
